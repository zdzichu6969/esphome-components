...
void EnergyStatistics::loop() {
  ...
  if (t.day_of_year == this->energy_production_.current_day_of_year) {
    // nothing to do
    return;
  }

  this->energy_production_.start_yesterday = this->energy_production_.start_today;

  this->energy_production_.start_today = total;

  if (this->energy_production_.current_day_of_year != 0) {
...
void EnergyStatistics::process_(float total) {
  if (this->energy_production_today_ && !std::isnan(this->energy_production_.start_today)) {
    this->energy_production_today_->publish_state(total - this->energy_production_.start_today);
  }

  if (this->energy_production_yesterday_ && !std::isnan(this->energy_production_.start_yesterday)) {
    this->energy_production_yesterday_->publish_state(this->energy_production_.start_today - this->energy_production_.start_yesterday);
  }

  if (this->energy_production_week_ && !std::isnan(this->energy_production_.start_week)) {
    this->energy_production_week_->publish_state(total - this->energy_production_.start_week);
  }

  if (this->energy_production_month_ && !std::isnan(this->energy_production_.start_month)) {
    this->energy_production_month_->publish_state(total - this->energy_production_.start_month);
  }
